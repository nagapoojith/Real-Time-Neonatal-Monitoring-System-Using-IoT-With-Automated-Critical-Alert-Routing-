...
): { status: string; color: string; bgColor: string; isNormal: boolean } => {
  if (value === undefined)
    return {
      status: "N/A",
      color: "#6b7280",
...
const getPositionInfo = (position?: string) => {
  switch (position) {
    case "back":
      return { label: "Supine (Back)", isSafe: true };
    case "side":
      return { label: "Lateral (Side)", isSafe: false };
    case "prone":
      return { label: "Prone (Stomach) - High Risk", isSafe: false };
    default:
      return { label: "Unknown", isSafe: true };
  }
};
...
const getAbnormalVitals = (vitals?: AlertEmailRequest["vitals"]): string[] => {
  ...
  if (!rr.isNormal)
    abnormals.push(`Respiration Rate: ${vitals.respirationRate}/min`);
...
async function generateAIContent(
  ...
  if (!apiKey) {
    console.log("AI_API_KEY missing, using fallback alert content");
...
function generateFallbackContent(
  ...
  if (!posInfo.isSafe) {
    return {
      ...
      closingMessage:
        "Always verify clinical alerts with direct patient assessment before initiating any interventions.",
    };
  }
...
function generateEmailHTML(
  ...
  const rrStatus = getVitalStatus(
    "respirationRate",
    request.vitals?.respirationRate,
  );
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      ...
      if (response.status === 400 || response.status === 401) {
        return {
          success: false,
          error: `Brevo API error (${response.status}): ${JSON.stringify(result)}`,
        };
      }
      ...
      if (attempt < maxRetries) {
        ...
        return {
          success: false,
          error: `Brevo API error after ${maxRetries} attempts: ${JSON.stringify(result)}`,
        };
      }
...
serve(async (req) => {
  ...
  try {
    ...
    const isHighPriority =
      request.alertType === "critical" || request.alertType === "high";
    console.log(
      `Alert type: ${request.alertType}, isHighPriority: ${isHighPriority}`,
    );

    const content = await generateAIContent(request, AI_API_KEY || "");
    ...
    console.log(
      `✅ ${request.alertType.toUpperCase()} alert email sent successfully to ${request.to}`,
    );
