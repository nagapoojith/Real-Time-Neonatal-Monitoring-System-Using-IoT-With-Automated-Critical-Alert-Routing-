...
const AlertCard: React.FC<AlertCardProps> = ({ alert, compact = false }) => {
  ...
  const levelConfig = {
    critical: {
      ...
      label: "Critical",
      bgClass:
        "bg-status-critical-bg/80 border-status-critical/30 dark:bg-[rgba(255,0,60,0.16)] dark:border-[rgba(255,64,96,0.45)]",
      iconBgClass: "bg-status-critical/15 dark:bg-status-critical/22",
      iconClass: "text-status-critical",
      messageClass:
        "bg-status-critical-bg/70 border-status-critical/25 dark:bg-[rgba(255,0,60,0.16)] dark:border-[rgba(255,64,96,0.45)]",
    },
    high: {
      icon: AlertTriangle,
      ...
      label: "High Priority",
      bgClass:
        "bg-status-warning-bg/80 border-status-warning/30 dark:bg-[rgba(255,170,0,0.16)] dark:border-[rgba(255,170,0,0.45)]",
      iconBgClass: "bg-status-warning/15 dark:bg-status-warning/20",
      iconClass: "text-status-warning",
      messageClass:
        "bg-status-warning-bg/70 border-status-warning/25 dark:bg-[rgba(255,170,0,0.16)] dark:border-[rgba(255,170,0,0.45)]",
    },
    normal: {
      icon: Bell,
      ...
      label: "Normal",
      bgClass:
        "bg-status-normal-bg/80 border-status-normal/30 dark:bg-status-normal-bg/25",
      iconBgClass: "bg-status-normal/15",
      iconClass: "text-status-normal",
      messageClass:
        "bg-status-normal-bg/70 border-status-normal/20 dark:bg-status-normal-bg/20",
    },
  };

  const config =
    levelConfig[alert.level as keyof typeof levelConfig] || levelConfig.normal;
  ...
  if (compact) {
    return (
      <div
        className={cn(
          "flex items-center gap-3 p-3 rounded-xl border transition-all duration-200 hover:shadow-sm border-l-[3px]",
          config.bgClass,
          alert.level === "critical" &&
            "border-l-status-critical shadow-[inset_0_0_12px_rgba(255,0,60,0.06)]",
          alert.level === "critical" && !alert.acknowledged && "alert-pulse",
        )}
      >
        <div
          className={cn("p-2.5 rounded-xl flex-shrink-0", config.iconBgClass)}
        >
  ...
  return (
    <Card
      className={cn(
        "alert-card overflow-hidden border",
        config.bgClass,
        alert.level === "critical" && "alert-card--critical",
        alert.level === "high" && "alert-card--warning",
        alert.level === "normal" && "alert-card--normal",
        !["critical", "high"].includes(alert.level) && "alert-card--normal",
        alert.level === "critical" &&
          !alert.acknowledged &&
          "alert-pulse dark:animate-glow-pulse",
      )}
    >
      <div
        className={cn(
      ...
      <CardContent className="p-5">
        <div className="flex items-start gap-4">
          ...
          <div className="flex-1 min-w-0">
            ...
            <p
              className={cn(
                "text-sm text-foreground mb-4 leading-relaxed p-3 rounded-xl border",
                config.messageClass,
              )}
            >
              {alert.message}
            ...
            <div className="flex items-center flex-wrap gap-3 text-xs text-muted-foreground">
              ...
              <Badge
                variant="outline"
                className="text-[10px] rounded-lg font-semibold"
              >
                {alert.type}
              </Badge>
              {emailSent && (
