...
const BabyCard: React.FC<BabyCardProps> = ({ baby }) => {
  ...
  const formatVitalValue = (type: string, value: number) => {
    switch (type) {
      case "temperature":
  ...
  return (
    <Link to={`/baby/${baby.id}`} className="block">
      <Card
        className={cn(
          "card-medical cursor-pointer group relative overflow-hidden glow-hover-primary",
          status.ringClass,
          status.pulseClass,
          baby.status === "critical" && "glow-hover-critical",
          baby.status === "high" && "glow-hover-warning",
        )}
      >
        <div
          className={cn(
              ...
              {[
                ...
                return (
                  <div
                    ...
                    className={cn(
                      ...
                      vitalStatus === "critical" &&
                        "border-status-critical/30 bg-status-critical-bg/30",
                      vitalStatus === "warning" &&
                  ...
                  >
                    ...
                    <p
                      className={cn(
                        "overflow-hidden whitespace-nowrap text-lg font-bold tabular-nums sm:text-xl",
                        getStatusStyles(vitalStatus),
                      )}
                    >
