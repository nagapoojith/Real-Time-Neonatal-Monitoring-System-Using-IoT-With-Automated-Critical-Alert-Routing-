...
const VitalsChart: React.FC<VitalsChartProps> = ({
  ...
  const fractionDigits = type === "temperature" ? 1 : 0;
  ...
  const spo2Min =
    numericValues.length > 0 ? Math.floor(Math.min(...numericValues)) - 2 : 90;
  const yDomain =
    type === "spo2"
      ? [Math.max(80, spo2Min), 100]
      : normalRange
        ? [normalRange.min - 10, normalRange.max + 10]
        : ["auto", "auto"];
