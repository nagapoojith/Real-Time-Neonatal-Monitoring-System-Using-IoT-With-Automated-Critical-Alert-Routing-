...
const ParentAuthContext = createContext<ParentAuthContextType | undefined>(
  undefined,
);
    ...
    async (
      ...
      try {
        ...
        if (!normalizedBabyName || !normalizedPassword) {
          return {
            success: false,
            error: "Please enter both Baby Name and Password.",
          };
        }
        ...
        const { data: babies, error } = await supabase
          .from("babies")
          .select(
            "id, baby_name, bed_number, parent_contact, status, login_password",
          )
        ...
        if (!babies || babies.length === 0) {
          return {
            success: false,
            error:
              "No baby found with this name. Please verify the information provided by the hospital.",
          };
        }
        ...
        if (!matchingBaby) {
          return {
            success: false,
            error:
              "Password does not match our records. Please check the password provided by the hospital.",
          };
        }
