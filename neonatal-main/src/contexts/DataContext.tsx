...
interface DataContextType {
  ...
  addBaby: (
    baby: Omit<
      Baby,
      "id" | "status" | "registeredAt" | "behaviorBaseline" | "alertsEnabled"
    >,
  ) => Promise<void>;
...
function checkVitalThresholds(vitals: VitalSigns): {
  level: AlertLevel;
  reasons: string[];
} {
  ...
  if (
    ...
    reasons.push(
      `Heart rate ${vitals.heartRate} BPM is CRITICAL (outside ${VITAL_THRESHOLDS.heartRate.criticalMin}-${VITAL_THRESHOLDS.heartRate.criticalMax} BPM)`,
    );
    level = "critical";
  } else if (
    vitals.heartRate < VITAL_THRESHOLDS.heartRate.highPriorityMin ||
    vitals.heartRate > VITAL_THRESHOLDS.heartRate.highPriorityMax
  ) {
    reasons.push(
      `Heart rate ${vitals.heartRate} BPM is HIGH PRIORITY (approaching limits)`,
    );
  ...
  if (
    vitals.respirationRate < VITAL_THRESHOLDS.respirationRate.criticalMin ||
    vitals.respirationRate > VITAL_THRESHOLDS.respirationRate.criticalMax
  ) {
    reasons.push(
      `Respiration rate ${vitals.respirationRate}/min is CRITICAL (outside ${VITAL_THRESHOLDS.respirationRate.criticalMin}-${VITAL_THRESHOLDS.respirationRate.criticalMax}/min)`,
    );
    level = "critical";
  } else if (
    vitals.respirationRate < VITAL_THRESHOLDS.respirationRate.highPriorityMin ||
    vitals.respirationRate > VITAL_THRESHOLDS.respirationRate.highPriorityMax
  ) {
    reasons.push(
      `Respiration rate ${vitals.respirationRate}/min is HIGH PRIORITY (approaching limits)`,
    );
    if (level === "normal") level = "high";
  }
  ...
  if (vitals.spo2 < VITAL_THRESHOLDS.spo2.critical) {
    reasons.push(
      `SpO₂ ${vitals.spo2}% is CRITICAL (below ${VITAL_THRESHOLDS.spo2.critical}%)`,
    );
    level = "critical";
  } else if (vitals.spo2 < VITAL_THRESHOLDS.spo2.warning) {
    reasons.push(
      `SpO₂ ${vitals.spo2}% is HIGH PRIORITY (below ${VITAL_THRESHOLDS.spo2.warning}%)`,
    );
    if (level === "normal") level = "high";
  }
  ...
  if (
    ...
    reasons.push(
      `Temperature ${vitals.temperature}°C is CRITICAL (outside ${VITAL_THRESHOLDS.temperature.criticalMin}-${VITAL_THRESHOLDS.temperature.criticalMax}°C)`,
    );
    level = "critical";
  } else if (
    vitals.temperature < VITAL_THRESHOLDS.temperature.highPriorityMin ||
    vitals.temperature > VITAL_THRESHOLDS.temperature.highPriorityMax
  ) {
    reasons.push(
      `Temperature ${vitals.temperature}°C is HIGH PRIORITY (approaching limits)`,
    );
    if (level === "normal") level = "high";
  }
...
export function DataProvider({ children }: { children: ReactNode }) {
        ...
        return {
          ...
          behaviorBaseline: {
            ...
            daysTracked: Math.floor(
              (Date.now() - new Date(baby.created_at).getTime()) /
                (24 * 60 * 60 * 1000),
            ),
  ...
  const fetchBabyRecipients = useCallback(
    async (babyId: string): Promise<string[]> => {
      try {
        const { data, error } = await supabase
        ...
        return (
          data
            ?.map((r) => r.email)
            .filter((e) => e && !e.includes("@placeholder.local")) || []
        );
      } catch (error) {
  ...
  const sendAutoAlertEmail = useCallback(
    async (
      baby: Baby,
      vitals: VitalSigns,
      level: AlertLevel,
      ...
      if (level === "normal") {
        console.log(
          `Alert level ${level} for ${baby.name} - no email sent (only CRITICAL and HIGH PRIORITY trigger emails)`,
        );
        return;
      }
      ...
      try {
        ...
        const emailTargets =
          emails.length > 0
            ? emails
            : [
                baby.parentContact.includes("@")
                  ? baby.parentContact
                  : "nrkavipriyan.cse2024@citchennai.net",
              ];
        ...
        const emailPromises = emailTargets.map(async (recipientEmail) => {
          try {
            ...
            if (emailError) {
              console.error(
                `Error sending alert to ${recipientEmail}:`,
                emailError,
              );
              return false;
            }
            console.log(
              `Auto alert sent to ${recipientEmail} for ${baby.name}`,
            );
            return true;
          } catch (error) {
  ...
  useEffect(() => {
    const rotationInterval = setInterval(() => {
      ...
      console.log(
        `Mock scenario rotated to: ${MOCK_SCENARIOS[mockScenarioIndex]}`,
      );
    }, 30000); // 30 seconds
  ...
  const addBaby = useCallback(
    async (
      ...
      const dateOfBirth = new Date(
        `${babyData.dateOfBirth}T${babyData.timeOfBirth}`,
      );
  ...
  const updateBabyStatus = useCallback(
    async (babyId: string, status: BabyStatus) => {
  ...
  const toggleBabyAlerts = useCallback(
    async (babyId: string, enabled: boolean) => {
      const { error } = await supabase
        .from("babies")
      ...
      setBabies((prev) =>
        prev.map((baby) =>
          baby.id === babyId ? { ...baby, alertsEnabled: enabled } : baby,
        ),
      );

      toast.success(
        `Alerts ${enabled ? "enabled" : "disabled"} for this patient`,
      );
    },
    [],
  );
  ...
  const deleteBaby = useCallback(
    async (babyId: string) => {
      ...
      toast.success(
        `${baby?.name || "Patient"} has been removed from monitoring`,
      );
    },
    [babies],
  );
  ...
  const acknowledgeAlert = useCallback(
    async (alertId: string, userName: string) => {
  ...
  const sendAlertEmail = useCallback(
    async (alert: Alert, recipientEmail: string, vitals?: VitalSigns) => {
      try {
        const currentVitals =
          vitals ||
          vitalsHistory[alert.babyId]?.[vitalsHistory[alert.babyId].length - 1];
