...
const VoiceAssistantCore: React.FC = () => {
  ...
  const sendToAI = useCallback(
    async (userText: string) => {
      ...
      const chatHistory = [...messagesRef.current, userMsg].map((m) => ({
        role: m.role,
        content: m.content,
      }));
      let assistantText = "";
  ...
  const statusConfig: Record<Status, { label: string; color: string }> = {
    ...
    listening: {
      label: "Listening...",
      color: "bg-status-normal/20 text-status-normal",
    },
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Clinical Mode — Speak to get NICU guidance
          </p>
        </div>
      ...
      {!speechSupported && (
        <Card className="portal-glow border-destructive bg-destructive/10">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        {/* Mic + Waveform */}
        <Card className="portal-glow flex flex-col items-center justify-center border-primary/15 py-12">
          <CardContent className="flex flex-col items-center gap-6">
            ...
            <div className="relative">
              {status === "listening" && (
                <>
                  <div
                    className="absolute inset-0 rounded-full bg-status-normal/20 animate-ping"
                    style={{ scale: "1.5" }}
                  />
                  <div
                    className="absolute inset-0 rounded-full bg-status-normal/10 animate-pulse"
                    style={{ scale: "1.8" }}
                  />
                </>
              )}
              <Button
                ...
                className={cn(
                  "portal-glow-soft h-24 w-24 rounded-full shadow-xl transition-all duration-300",
                  status === "listening"
            ...
            {transcript && (
              <Card className="portal-glow-soft w-full border-border/50 bg-muted/50">
        ...
        <Card className="portal-glow border-primary/15">
              ...
              <div className="p-4 space-y-4">
                ...
                {messages.map((msg, i) => (
                  ...
                  >
                    <div
                      className={cn(
                        "portal-glow-soft max-w-[85%] rounded-2xl px-4 py-3",
                        msg.role === "user"
